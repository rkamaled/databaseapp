...
const FilterSection = ({ onSearch }) => {
  ...
  // Update a specific field on a logic parameter
  const updateLogicParameterField = (filterId, logicParamId, field, value) => {
    setFilters(prev => prev.map(f => 
      f.id === filterId 
        ? { 
            ...f, 
  ...
  const updateLogicParameterCohorts = (filterId, logicParamId, cohorts) => {
    setFilters(prev => prev.map(f => {
      if (f.id === filterId) {
        ...
        // For each selected cohort, fetch variables if we haven't already (skip for Genotype Data)
  ...
  const handleSubmit = (e) => {
    ...
    const processedFilters = filters.map(filter => {
      if (filter.modality === 'Genotype Data') {
        const lp = (filter.logicParameters && filter.logicParameters[0]) || { cohorts: [], snp_input_text: '' };
        const snpTokens = (lp.snp_input_text || '')
          .split(/[,\s]+/)
          .map(s => s.trim())
          .filter(Boolean);
        const snpList = snpTokens.length > 0 ? [snpTokens[0]] : [];
        return {
          modality: 'genetic',
          logicParameters: [
            {
  ...
  const isSearchDisabled = () => {
    ...
    if (filters.some(filter => !filter.logicParameters || filter.logicParameters.length === 0)) {
      return true;
    }
    // For genotype, require at least one cohort and at least one SNP
    const genotypeInvalid = filters.some(f => {
      if (f.modality !== 'Genotype Data') return false;
      const lp = f.logicParameters[0];
      const hasCohort = lp && Array.isArray(lp.cohorts) && lp.cohorts.length > 0;
      const tokens = (lp && typeof lp.snp_input_text === 'string')
        ? lp.snp_input_text.split(/[,\s]+/).map(s => s.trim()).filter(Boolean)
        : [];
      const exactlyOneSnp = tokens.length === 1;
      return !(hasCohort && exactlyOneSnp);
    });
    if (genotypeInvalid) return true;
    return false;
  };
                ...
                <div className="logic-parameters-section">
                  ...
                  {filter.logicParameters.map((logicParam) => (
                    <div key={logicParam.id} className="logic-parameter-card">
                      ...
                      {/* Genotype special case: SNP input replaces variables/timepoints/thresholds */}
                      {filter.modality === 'Genotype Data' ? (
                        <div className="param-group">
                          <label>SNP (rsID): <span className="required-field">*</span></label>
                          <input
                            type="text"
                            value={logicParam.snp_input_text}
                            onChange={(e) => updateLogicParameterField(filter.id, logicParam.id, 'snp_input_text', e.target.value)}
                            placeholder="Enter a single rsID (e.g., rs123456)"
                            className="variable-search-input"
                          />
                          {(() => {
                            const tokens = (logicParam.snp_input_text || '').split(/[,\s]+/).map(s => s.trim()).filter(Boolean);
                            if (tokens.length === 0) {
                              return <div className="validation-message">Please enter one rsID</div>;
                            }
                            if (tokens.length > 1) {
                              return <div className="validation-message">Please enter only one rsID</div>;
                            }
                            return null;
                          })()}
                        </div>
